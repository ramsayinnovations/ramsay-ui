...
const select = {
  wrapper: tw`w-full max-w-xs mx-auto`,
  select: tw`space-y-1`,
  label: tw`block text-sm leading-5 font-medium text-gray-700`,
  main: tw`relative`,
  button: {
    wrapper: tw`inline-block w-full rounded-md `,
    base: tw`cursor-default relative w-full rounded-md border border-gray-300 bg-white pl-3 pr-10 py-2 text-left focus:outline-none focus:border-blue-300 transition ease-in-out duration-150 sm:text-sm sm:leading-5`,
  ...
  transition: tw`absolute mt-1 w-full rounded-md bg-white `,
  listBox: {
    base: tw`max-h-60 rounded-md py-1 text-base leading-6  overflow-auto focus:outline-none sm:text-sm sm:leading-5`,
    item: {
